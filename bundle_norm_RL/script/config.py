"""
公共配置文件，包含 trial point 和问题参数
"""

import json
import pickle
from pathlib import Path

from sddip.sddip import parameters



class LevelBundleConfig:
    """Level Bundle 配置类，包含所有参数"""

    def __init__(
        self,
        T,
        N_VARS,
        X_TRIAL,
        Y_TRIAL,
        X_BS_TRIAL,
        SOC_TRIAL,
        THETA_TRIAL,
        PATH,
        iteration: int = 0,
        bc_storage=None,
        dual_solver_storage=None,
        n: int = 0,  # realization 索引
        # Level Bundle 专用参数
        level_factor: float = 0.3,
        iteration_limit: int = 200,
        gap_tol: float = 5e-3,
        pi0_tol: float = 1e-8,
        time_limit: int = 3600,  # 秒
    ):
        self.T = T
        self.N_VARS = N_VARS
        self.X_TRIAL = X_TRIAL
        self.Y_TRIAL = Y_TRIAL
        self.X_BS_TRIAL = X_BS_TRIAL
        self.SOC_TRIAL = SOC_TRIAL
        self.THETA_TRIAL = THETA_TRIAL
        self.PATH = PATH if isinstance(PATH, str) else str(PATH)
        self.PROBLEM_PARAMS = parameters.Parameters(Path(PATH))
        self.iteration = iteration  # 当前迭代次数 i
        self.bc_storage = bc_storage  # Benders cuts 存储
        self.dual_solver_storage = dual_solver_storage  # Lagrangian cuts 存储
        self.n = n  # realization 索引
        # Level Bundle 专用参数
        self.level_factor = level_factor
        self.iteration_limit = iteration_limit
        self.gap_tol = gap_tol
        self.pi0_tol = pi0_tol
        self.time_limit = time_limit

    @property
    def trial_point(self):
        return (self.X_TRIAL, self.Y_TRIAL, self.X_BS_TRIAL, self.SOC_TRIAL)

    @property
    def X_trial(self):
        """拼接后的 trial point: x + y + x_bs_flat + soc"""
        return (
            self.X_TRIAL
            + self.Y_TRIAL
            + [val for bs in self.X_BS_TRIAL for val in bs]
            + self.SOC_TRIAL
        )

    def to_dict(self):
        """将对象转换为字典，用于JSON序列化"""
        return {
            "T": self.T,
            "N_VARS": self.N_VARS,
            "X_TRIAL": self.X_TRIAL,
            "Y_TRIAL": self.Y_TRIAL,
            "X_BS_TRIAL": self.X_BS_TRIAL,
            "SOC_TRIAL": self.SOC_TRIAL,
            "THETA_TRIAL": self.THETA_TRIAL,
            "PATH": self.PATH,
            "iteration": self.iteration,
            "n": self.n,
            "level_factor": self.level_factor,
            "iteration_limit": self.iteration_limit,
            "gap_tol": self.gap_tol,
            "pi0_tol": self.pi0_tol,
            "time_limit": self.time_limit,
            "bc_storage_type": type(self.bc_storage).__name__ if self.bc_storage else None,
            "dual_solver_storage_type": type(self.dual_solver_storage).__name__ if self.dual_solver_storage else None,
        }

    def toString(self):
        """返回JSON字符串表示"""
        return json.dumps(self.to_dict(), ensure_ascii=False, indent=2)

    @classmethod
    def fromString(cls, json_str):
        """从JSON字符串解析出LevelBundleConfig对象"""
        data = json.loads(json_str)
        return cls(
            T=data["T"],
            N_VARS=data["N_VARS"],
            X_TRIAL=data["X_TRIAL"],
            Y_TRIAL=data["Y_TRIAL"],
            X_BS_TRIAL=data["X_BS_TRIAL"],
            SOC_TRIAL=data["SOC_TRIAL"],
            THETA_TRIAL=data["THETA_TRIAL"],
            PATH=data["PATH"],
            iteration=data.get("iteration", 0),
            n=data.get("n", 0),
            level_factor=data.get("level_factor", 0.3),
            iteration_limit=data.get("iteration_limit", 200),
            gap_tol=data.get("gap_tol", 5e-3),
            pi0_tol=data.get("pi0_tol", 1e-4),
            time_limit=data.get("time_limit", 3600),
        )

    def to_pkl(self, file_path):
        """保存为pickle文件"""
        with open(file_path, "wb") as f:
            pickle.dump(self, f)

    @classmethod
    def from_pkl(cls, file_path):
        """从pickle文件加载LevelBundleConfig对象"""
        with open(file_path, "rb") as f:
            return pickle.load(f)


# 问题参数
T = 5
N_VARS = 13

# Trial point
X_TRIAL = [-0.0, 1.0, 1.0]
Y_TRIAL = [0.0, 131.60809087723158, 45.0]
X_BS_TRIAL = [[-0.0, 0.0], [1.0, 1.0], [1.0, 1.0]]
SOC_TRIAL = [0.0]
THETA_TRIAL = 0.0


def get_default_level_bundle_config() -> LevelBundleConfig:
    """获取默认的 LevelBundleConfig"""
    return LevelBundleConfig(
        T=T,
        N_VARS=N_VARS,
        X_TRIAL=X_TRIAL,
        Y_TRIAL=Y_TRIAL,
        X_BS_TRIAL=X_BS_TRIAL,
        SOC_TRIAL=SOC_TRIAL,
        THETA_TRIAL=THETA_TRIAL,
        PATH=Path(r"D:\tools\workspace_pycharm\SDDiP-RL\data\01_test_cases\case6ww\t06_n06")
    )


class LevelBundleConfigNorm(LevelBundleConfig):
    """
    Level Bundle 配置类 (带正则化策略与范数边界约束)

    新增参数:
        sigma_t: 正则化系数 σ_t (默认 10.0)
        B_t: 对偶边界值 (默认等于 sigma_t)
        norm_type: 正则化范数类型 "l1" 或 "linf" (默认 "l1")
        norm_bound_type: 范数边界约束类型 "l1" 或 "linf" (默认 "l1")
        weights: 权重系数列表 (默认 None, 即全为 1)
        _skip_problem_params_init: 内部使用，跳过 PROBLEM_PARAMS 初始化
    """

    def __init__(
        self,
        T,
        N_VARS,
        X_TRIAL,
        Y_TRIAL,
        X_BS_TRIAL,
        SOC_TRIAL,
        THETA_TRIAL,
        PATH,
        iteration: int = 0,
        bc_storage=None,
        dual_solver_storage=None,
        n: int = 0,
        level_factor: float = 0.3,
        iteration_limit: int = 200,
        gap_tol: float = 5e-3,
        pi0_tol: float = 1e-8,
        time_limit: int = 3600,
        # 正则化与范数边界参数
        sigma_t: float = 10.0,
        B_t: float = None,
        norm_type: str = "l1",
        norm_bound_type: str = "l1",
        weights: list = None,
        # 内部参数
        _skip_problem_params_init: bool = False,
    ):
        if _skip_problem_params_init:
            # 跳过 super().__init__，手动初始化父类属性
            self.T = T
            self.N_VARS = N_VARS
            self.X_TRIAL = X_TRIAL
            self.Y_TRIAL = Y_TRIAL
            self.X_BS_TRIAL = X_BS_TRIAL
            self.SOC_TRIAL = SOC_TRIAL
            self.THETA_TRIAL = THETA_TRIAL
            self.PATH = PATH if isinstance(PATH, str) else str(PATH)
            self.PROBLEM_PARAMS = None  # 稍后覆盖
            self.iteration = iteration
            self.bc_storage = bc_storage
            self.dual_solver_storage = dual_solver_storage
            self.n = n
            self.level_factor = level_factor
            self.iteration_limit = iteration_limit
            self.gap_tol = gap_tol
            self.pi0_tol = pi0_tol
            self.time_limit = time_limit
        else:
            super().__init__(
                T=T,
                N_VARS=N_VARS,
                X_TRIAL=X_TRIAL,
                Y_TRIAL=Y_TRIAL,
                X_BS_TRIAL=X_BS_TRIAL,
                SOC_TRIAL=SOC_TRIAL,
                THETA_TRIAL=THETA_TRIAL,
                PATH=PATH,
                iteration=iteration,
                bc_storage=bc_storage,
                dual_solver_storage=dual_solver_storage,
                n=n,
                level_factor=level_factor,
                iteration_limit=iteration_limit,
                gap_tol=gap_tol,
                pi0_tol=pi0_tol,
                time_limit=time_limit,
            )
        self.sigma_t = sigma_t
        self.B_t = B_t if B_t is not None else sigma_t  # 默认 B_t = σ_t
        self.norm_type = norm_type
        self.norm_bound_type = norm_bound_type
        self.weights = weights  # None 表示全为 1

    def to_dict(self):
        """将对象转换为字典，用于JSON序列化"""
        d = super().to_dict()
        d.update({
            "sigma_t": self.sigma_t,
            "B_t": self.B_t,
            "norm_type": self.norm_type,
            "norm_bound_type": self.norm_bound_type,
            "weights": self.weights,
        })
        return d

    def toString(self):
        """返回JSON字符串表示"""
        return json.dumps(self.to_dict(), ensure_ascii=False, indent=2)

    @classmethod
    def from_base_config(cls, base_config: LevelBundleConfig, **kwargs):
        """
        从已有的 LevelBundleConfig 创建 LevelBundleConfigNorm

        直接复用 base_config 中已初始化的 PROBLEM_PARAMS，
        避免因 PATH 为相对路径而重新加载参数文件失败。

        Args:
            base_config: 基础配置对象
            **kwargs: 正则化与范数边界参数 (sigma_t, B_t, norm_type, norm_bound_type, weights)

        Returns:
            LevelBundleConfigNorm
        """
        obj = cls(
            T=base_config.T,
            N_VARS=base_config.N_VARS,
            X_TRIAL=base_config.X_TRIAL,
            Y_TRIAL=base_config.Y_TRIAL,
            X_BS_TRIAL=base_config.X_BS_TRIAL,
            SOC_TRIAL=base_config.SOC_TRIAL,
            THETA_TRIAL=base_config.THETA_TRIAL,
            PATH=base_config.PATH,
            iteration=base_config.iteration,
            bc_storage=base_config.bc_storage,
            dual_solver_storage=base_config.dual_solver_storage,
            n=base_config.n,
            level_factor=base_config.level_factor,
            iteration_limit=base_config.iteration_limit,
            gap_tol=base_config.gap_tol,
            pi0_tol=base_config.pi0_tol,
            time_limit=base_config.time_limit,
            _skip_problem_params_init=True,  # 跳过 PROBLEM_PARAMS 初始化
            **kwargs,
        )
        # 直接复用已初始化的 PROBLEM_PARAMS，避免相对路径问题
        obj.PROBLEM_PARAMS = base_config.PROBLEM_PARAMS
        return obj

    def to_pkl(self, file_path):
        """保存为pickle文件"""
        with open(file_path, "wb") as f:
            pickle.dump(self, f)

    @classmethod
    def from_pkl(cls, file_path):
        """从pickle文件加载LevelBundleConfigNorm对象"""
        with open(file_path, "rb") as f:
            return pickle.load(f)


def get_default_level_bundle_config_norm() -> LevelBundleConfigNorm:
    """获取默认的 LevelBundleConfigNorm"""
    return LevelBundleConfigNorm(
        T=T,
        N_VARS=N_VARS,
        X_TRIAL=X_TRIAL,
        Y_TRIAL=Y_TRIAL,
        X_BS_TRIAL=X_BS_TRIAL,
        SOC_TRIAL=SOC_TRIAL,
        THETA_TRIAL=THETA_TRIAL,
        PATH=Path(r"D:\tools\workspace_pycharm\SDDiP-RL\data\01_test_cases\case6ww\t06_n06"),
        sigma_t=10.0,
        B_t=10.0,
        norm_type="l1",
        norm_bound_type="l1",
    )