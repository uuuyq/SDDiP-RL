"""
PPO 训练函数 for Incremental Level Bundle RL

使用 SB3 原生 MultiInputPolicy + 自定义 IncrementalLevelBundleFeaturesExtractor。
与 script/train.py 保持相同的风格和目录结构。
"""
...
def load_latest_checkpoint(experiment_name, env, policy_kwargs):
    ...
    checkpoints = [f for f in os.listdir(checkpoints_dir)
                   if f.startswith("ppo_incremental_level_bundle_checkpoint") and f.endswith(".zip")]
...
def train(
    ...
    experiment_name="incremental_level_bundle_default",
    ent_coef=0,
    resume=True,
    overwrite=False,
    learning_rate=1e-5,
    ...
    """
    Incremental Level Bundle PPO 训练函数
    ...
    checkpoint_callback = CheckpointCallback(
        ...
        name_prefix="ppo_incremental_level_bundle_checkpoint",
    )
    ...
    model_name = save_path if save_path else f"ppo_incremental_level_bundle_{timestamp}"
