"""
公共配置文件，包含 trial point 和问题参数（增量形式）

与 script/config.py 共享 LevelBundleConfig，但增加增量形式专用参数:
    rho: proximal 惩罚参数
    B_t: 对偶边界值
    norm_bound_type: 范数边界约束类型
    weights: 权重系数列表
"""
...
class LevelBundleConfig:
    ...
    def fromString(cls, json_str):
        ...
        return cls(
            ...
            B_t=data.get("B_t", None),
            norm_bound_type=data.get("norm_bound_type", "l1"),
        )
