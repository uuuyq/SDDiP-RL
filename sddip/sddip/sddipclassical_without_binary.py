...
class Algorithm:
    ...
    def run(self, n_iterations: int) -> None:
        ...
        self.current_cut_mode = self.secondary_cut_mode  # 固定为 lag
        ...
        for t in reversed(range(1, self.problem_params.n_stages)):
            for k in range(n_samples):
                ...
                for n in range(n_realizations):
                    ...
                    # uc_fw.model.write(f"uc_fw_model_{t}.lp")
                    # if t != self.problem_params.n_stages - 1:
                    #     benders_coefficients = self.bc_storage.get_stage_result(t)
                    #     print(f"t {t}, n {n}  n_cuts:{len(benders_coefficients[ResultKeys.bc_intercept_key])}")
                    # print(f"cuts {self.bc_storage.get_stage_result(t)}")
                    # print("uc_fw obj", uc_fw.model.getObjective().getValue())
                    # print(f"t: {t}, n: {n} dm: {dm}")
                    ...
                    if self.current_cut_mode == common.CutType.BENDERS:
                        ...
                        dual_model.relaxed_terms_calculate_without_binary(
                            x_trial_point,
                            y_trial_point,
                            x_bs_trial_point,
                            soc_trial_point,
                        ...
                        # uc_fw.model.write(f"dual_model_{t}.lp")
