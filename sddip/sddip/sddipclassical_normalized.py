        ...
        for t in reversed(range(1, self.problem_params.n_stages)):
            for k in range(n_samples):
                ...
                for n in range(n_realizations):
                    ...
                    relaxed_terms = [-relaxed_terms[j] for j in range(len(relaxed_terms))]
                    # objective_terms - theta_trial_point
                    relaxed_terms.append(objective_terms - theta_trial_point)
                    ...
                    self.len_trial_points = len(binary_trial_point)
                    ...
                    dual_value = sg_results.obj_value + pi0 * theta_trial_point + sum(
                        [
                            pi[j] * binary_trial_point[j]
                            for j in range(len(binary_trial_point))
                        ]
                    )
