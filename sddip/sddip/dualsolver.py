...
# logger.setLevel(logging.DEBUG)  # 设置日志等级（DEBUG/INFO/WARNING/ERROR/CRITICAL）
#
# # 创建一个输出到控制台的 handler
...
class BundleMethod(DualSolver):
    ...
    def create_normalized_subproblem(
        self, n_dual_multipliers: int
    ) -> tuple[gp.Model, gp.Var, gp.tupledict]:
        """Create the bundle method's subproblem."""
        subproblem = gp.Model("Subproblem")
        ...
        # pi0 > 0
        subproblem.addConstr(x[n_dual_multipliers - 1] >= 0, name=f"pi0>0")

        # L1 norm
        abs_x = []
        for i in range(n_dual_multipliers - 1):
            abs_x.append(subproblem.addVar(vtype=gp.GRB.CONTINUOUS, lb=0, name=f"abs_x_{i}"))
            subproblem.addConstr(abs_x[i] >= x[i], name=f"+abs_{i}")
            subproblem.addConstr(abs_x[i] >= -x[i], name=f"-abs_{i}")
        subproblem.addConstr(
            gp.quicksum(abs_x[i] for i in range(n_dual_multipliers - 1)) + x[n_dual_multipliers - 1] == 1, name=f"L1_norm"
        )
        return subproblem, v, x
