...
class ModelBuilder(ABC):
    ...
    def initialize_copy_variables(self) -> None:
        ...
        """
        注意：z_x和z_x_bs的lb=0,ub=1是额外添加的，
        不二元化的方式如果不添加这个限制，SB cuts会出错，LB无限制的上升，多倍的超过上界
        对于二元化的方式，由于z_x == x_bin_copy_vars约束的存在，z_x变量被间接的限制在0和1之间
        我认为增加这个限制是合理的
        这个限制有无对于lag的计算目前来看是无影响的
        """
    ...
    def add_inner_objective(self, coefficients: list, pi_hat, pi0_hat):
        ...
        self.model.setObjective(gp.LinExpr(pi_hat, z_X) + pi0_hat * obj_term)
        self.model.update()
        return z_X, obj_term
