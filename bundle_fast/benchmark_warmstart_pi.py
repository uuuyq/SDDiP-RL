"""
benchmark_warmstart_pi.py

对比两种求解方式：
1. warmstart_pi: 使用 fast_g_gen 生成的 subgradients 加权求和得到一个 pi，作为 bundle 算法的起点
...
def run_bundle_warmstart_pi(
    ...
    """
    if logger is None:
        logger = get_logger("log/benchmark_warmstart_pi.log")

    trial_point = (config.X_TRIAL, config.Y_TRIAL, config.X_BS_TRIAL, config.SOC_TRIAL)
    problem_params = config.PROBLEM_PARAMS
    t = config.T

    # ========== 第一步：生成 pi（调用 fast_g_gen）==========


    logger.info(">>> 步骤1: 历史解收集 (fast_g_gen.history_solution_collect) <<<")
    mu_weights, solution_collection = fast_g_gen.history_solution_collect()
    pi_gen_start_time = time.time()
    logger.info(">>> 步骤2: 生成 subgradients (fast_g_gen.bundle_fast) <<<")
    subgradients, mu_list, z_vars_list, x_vars_list = fast_g_gen.gen_subgradient(mu_weights, solution_collection, solution_size)

    # 加权求和得到一个 pi
    logger.info(">>> 步骤3: 加权求和生成 pi <<<")
    subgradients_np = np.array(subgradients)
    mu_np = np.array(mu_list)
    pi = np.sum(mu_np[:, np.newaxis] * subgradients_np, axis=0)

    pi_gen_time = time.time() - pi_gen_start_time
    logger.info(f"Pi 生成耗时: {pi_gen_time:.3f}s")
    logger.info(f"生成的 pi: {pi}")

    # ========== 第二步：使用 pi 进行 bundle 迭代 ==========
    ...
    # 使用 pi 求解子问题，得到初始 f 和 g
    g_new, f_new = sub.solve(pi)

    # 记录初始点：下界为子问题求解值，上界设为一个很大的值
    ub_history.append(1e4)
    lb_history.append(f_new)
    iter_times.append(time.time() - bundle_start_time)
    logger.info(f"Warmstart Pi 初始: lb={f_new:.6f}, ub=1e4 (初始值)")

    # 初始化 f_best 和 x_best
    master.f_best = f_new
    master.x_best = pi.copy()

    # 继续迭代
    for i in range(max_iterations):
        iter_start = time.time()

        master.add_cut(pi, f_new, g_new)
        ub, pi = master.solve_master()
        g_new, f_new = sub.solve(pi)
        serious_step, delta, stop_flag = master.update_strategy(pi, f_new, g_new, ub)
        ...
        if stop_flag:
            logger.info(f"Warmstart Pi 收敛于迭代 {i+1}")
            break

    bundle_time = time.time() - bundle_start_time
    total_time = pi_gen_time + bundle_time

    result = {
        "method": "warmstart_pi",
        "pi_gen_time": pi_gen_time,
...
def main(
    ...
    logger = get_logger("log/benchmark_warmstart_pi.log")
    logger.info("=" * 50)
    logger.info("开始 Benchmark 对比测试 (Warmstart Pi vs Baseline)")
    logger.info(f"参数: t={config.T}, max_iter={max_iterations}, tol={tolerance}, solution_size={solution_size}, realization={realization}")
    logger.info("=" * 50)

    # 运行 warmstart_pi 方法
    logger.info("\n>>> 运行 Warmstart Pi 方法 <<<")
    result_warmstart = run_bundle_warmstart_pi(
    ...
    # 保存结果
    save_result(result_warmstart, f"benchmark_warmstart_pi_{config.T}_{realization}.json")
    save_result(result_baseline, f"benchmark_baseline_{config.T}_{realization}.json")

    # 绘制收敛曲线
    plot_convergence(result_warmstart, f"benchmark_warmstart_pi_{config.T}_{realization}.png")
    ...
    print("对比摘要")
    print("=" * 60)
    print(f"{'指标':<25} {'Warmstart Pi':<20} {'Baseline':<15}")
    print("-" * 60)
    print(f"{'迭代次数':<25} {result_warmstart['iterations']:<20} {result_baseline['iterations']:<15}")
    print(f"{'Pi生成耗时 (s)':<25} {result_warmstart['pi_gen_time']:.3f}{'':<16} {'-':<15}")
