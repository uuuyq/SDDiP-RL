"""
Attention Encoder 模块（简化版）

该模块定义了 Attention1 简化版的编码器组件，用于从观测中提取特征。
...
输出 (供 Policy 使用):
├── cut_embeddings: (batch_size, K, hidden_dim) - 编码后的cut序列 (即 H)
├── global_embedding: (batch_size, hidden_dim) - 全局信息编码
└── cls_embedding: (batch_size, hidden_dim) - 序列级表示 (即 h_bundle)
...
class AttentionBundleEncoder(nn.Module):
    """
    Attention-based Bundle Encoder（简化版）

    整合 CutEncoder、SelfAttention、GlobalEncoder

    输出三元组：
        H        = cut_embeddings   (B, K, hidden_dim)   # 每个 cut 的精细化表示
        h_bundle = cls_embedding    (B, hidden_dim)      # bundle 整体表示（CLS）
        global   = global_embedding (B, hidden_dim)      # 全局上下文
    """
    ...
    def forward(
        ...
        Returns:
            cut_embeddings: shape (batch_size, K, hidden_dim)        # H
            global_embedding: shape (batch_size, hidden_dim)         # global
            cls_embedding: shape (batch_size, hidden_dim)            # h_bundle
        """
