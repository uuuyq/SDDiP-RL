...
# 导入 eta 模块专用的测试函数
from bundle_RL.script.eta.test import bundle_baseline, bundle_RL, bundle_RL_warmstart
...
def plot_results(avg_results, save_dir, experiment_name=None, tb_writer=None):
    ...
    # 检查是否有数据
    has_data = (len(avg_results["baseline"]["rel_gap"]) > 0 or 
                len(avg_results["rl"]["rel_gap"]) > 0 or 
                len(avg_results["rl_warmstart"]["rel_gap"]) > 0)
    ...
    if all_rel_gaps:
        ...
        y_max = max(all_rel_gaps) * 1.1
    else:
        y_min, y_max = 0, 1

    # 图1：gap随迭代次数的收敛图
    plt.figure(figsize=(8, 5))
    plt.plot(avg_results["baseline"]["rel_gap"], marker='s', color='#2196F3', label='Baseline', linewidth=2)
    plt.plot(avg_results["rl"]["rel_gap"], marker='o', color='#FF5722', label='RL', linewidth=2)
    plt.plot(avg_results["rl_warmstart"]["rel_gap"], marker='^', color='#4CAF50', label='RL Warmstart', linewidth=2)
    ...
    if tb_writer is not None:
        ...
        if img_array.ndim == 3 and img_array.shape[-1] == 4:  # RGBA to RGB
            img_array = img_array[..., :3]
        if img_array.ndim == 3:
            img_array = np.transpose(img_array, (2, 0, 1))
        tb_writer.add_image('convergence_iteration', img_array, 0)
        print("已添加 convergence_iteration 图像到 TensorBoard")
    ...
    plt.plot(baseline_cum_time, avg_results["baseline"]["rel_gap"], marker='s', color='#2196F3', label='Baseline', linewidth=2)
    plt.plot(rl_cum_time, avg_results["rl"]["rel_gap"], marker='o', color='#FF5722', label='RL', linewidth=2)
    plt.plot(rl_warmstart_cum_time, avg_results["rl_warmstart"]["rel_gap"], marker='^', color='#4CAF50', label='RL Warmstart', linewidth=2)
    ...
    if tb_writer is not None:
        ...
        if img_array.ndim == 3:
            img_array = np.transpose(img_array, (2, 0, 1))
        tb_writer.add_image('convergence_time', img_array, 0)
        print("已添加 convergence_time 图像到 TensorBoard")
    
    plt.close()

    print(f"Convergence plots saved to {save_dir}")
    
    # 同时保存数值数据到 TensorBoard
    if tb_writer is not None:
...
def load_latest_model(train_experiment_name, logger):
    ...
    # 上移两级目录：bundle_RL/script/eta/ -> bundle_RL/script/ -> bundle_RL/
    ...
    try:
        from bundle_RL.script.eta.train import SimpleBundleExtractor
...
def run_test_for_configs(configs, config_info_list, experiment_name, logger, model,
    ...
    """
    对一组 config 运行测试，计算相对 gap（eta 版本）
    """
    all_results = []
    ...
    for idx, (config, config_info) in enumerate(zip(configs, config_info_list)):
        ...
        test_env, _ = BundleDualEnv.create_env(logger, config, tolerance=tolerance, verbose=True, K=K)
        rl_delta, rl_reward, rl_time, rl_ub, rl_f_best = bundle_RL(
            test_env, model, logger, deterministic=True)
        rl_switch_step = None

        logger.info("Running RL Warmstart...")
        warmstart_env, _ = BundleDualEnv.create_env(logger, config, tolerance=tolerance, verbose=True, K=K)
        ws_delta, ws_reward, ws_time, ws_ub, ws_f_best, ws_switch_step = bundle_RL_warmstart(
            warmstart_env, model, logger,
...
if __name__ == "__main__":
    ...
    tb_root_dir = os.path.join(base_dir, "test_result", "tb_summary")
    ...
    print(f"\n{'=' * 60}")
    print(f"所有测试完成!")
    print(f"TensorBoard 日志已保存至: {tb_root_dir}")
    print(f"运行以下命令查看:")
