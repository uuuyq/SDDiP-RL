...
class SimpleBundleExtractor(BaseFeaturesExtractor):
    ...
    def forward(self, observations):
        # 把所有的标量特征连接起来
        log_eta = observations["log_eta"]
        log_delta = observations["log_delta"]
        serious_step = observations["serious_step"]
        d_norm_sq = observations["d_norm_sq"]
        proximal_term = observations["proximal_term"]
        lin_error = observations["lin_error"]
        gap_improve_1 = observations["gap_improve_1"]
        gap_improve_2 = observations["gap_improve_2"]
        gap_improve_3 = observations["gap_improve_3"]
        g_norm_sq = observations["g_norm_sq"]
        cos_gd = observations["cos_gd"]

        print(f"log_data: {log_eta}")
        print(f"log_data: {log_delta}")
        print(f"serious_step: {serious_step}")
        print(f"d_norm_sq: {d_norm_sq}")
        print(f"proximal_term: {proximal_term}")
        print(f"lin_error: {lin_error}")
        print(f"gap_improve_1: {gap_improve_1}")
        print(f"gap_improve_2: {gap_improve_2}")
        print(f"gap_improve_3: {gap_improve_3}")
        print(f"g_norm_sq: {g_norm_sq}")
        print(f"cos_gd: {cos_gd}")

        x = torch.cat([
            log_eta, log_delta, serious_step, d_norm_sq, proximal_term,
            lin_error, gap_improve_1, gap_improve_2, gap_improve_3,
            g_norm_sq, cos_gd
        ], dim=1)
        return self.net(x)
