...
def get_experiment_dirs(experiment_name):
    ...
    # 上移两级目录：bundle_RL/script/deepset/ -> bundle_RL/script/ -> bundle_RL/
...
def load_latest_checkpoint(experiment_name, env, policy_kwargs):
    ...
    model = PPO.load(
        ...
        env=env,
        custom_objects={
            "DeepSetBundleExtractor": DeepSetBundleExtractor,
...
def train(env, save_path=None, logger=None, model=None, total_timesteps=200_000,
          checkpoint_freq=5000, experiment_name="deepset_default", ent_coef=0,
          ...
          features_dim=128, hidden_dim=64, dropout=0.1,
          actor_net_arch=None, critic_net_arch=None,
          policy_type="deepset"):
    """
    DeepSet-based PPO 训练函数，支持断点续训
    ...
    # 特征提取器参数（新版：只需要 hidden_dim，query/key 在策略中定义）
    ...
    policy_kwargs = dict(
        features_extractor_class=DeepSetBundleExtractor,
        features_extractor_kwargs=features_extractor_kwargs,
        net_arch=dict(pi=actor_net_arch, vf=critic_net_arch),
        query_dim=hidden_dim,  # Query 维度等于 hidden_dim
        key_dim=hidden_dim     # Key 维度等于 hidden_dim
    )
