...
"""
DeepSet-based Bundle Environment

state：当前的所有cuts，valid_mask，当前的pi值，当前的trial_point、以及场景 realization
action：cut weights 和 步长

状态转移：weights + 步长 -> 归一化 -> pi -> sub求解得到子问题
...
class BundleDualEnv(gym.Env):
    def __init__(self, logger, config, n, state_dim, K, verbose=False):
        ...
        :param K: 使用padding的方式，K代表最大的cuts数，同时也是输出的weights维度
        ...
        self.action_dim = K + 1  # 输出weights以及步长
        self.logger = logger
    ...
    def step(self, action):
        ...
        weight_sum = np.sum(masked_weights) + 1e-8
        normalized_weights = masked_weights / weight_sum

        d = normalized_weights @ G  # (state_dim,)
